  ...
  _createClass(Service, [{
    ...
    value: function find(params) {
      ...
      var getResults = function getResults() {
        return filter.limit === 0 ? Promise.resolve([]) : _this.model.find(filter).then(_util.jsonify);
      };
      if (!paginate.default) {
        return getResults();
      }
      var total = void 0;
      return this.model.count(filter.where).then(function (count) {
        total = count;
        return getResults();
      }).then(function (data) {
        return {
    ...
    value: function get(id, params) {
      ...
      return this.model.findById(id, filter).then(function (result) {
        ...
        return result;
      }).then(_util.jsonify).then(select);
    }
  }, {
    key: 'create',
    value: function create(data, params) {
      var select = _feathersCommons2.default.select(params, this.id);
      return this.model.create(data).then(_util.jsonify).then(select);
    }
  }, {
    key: 'update',
    value: function update(id, data, params) {
      ...
      delete data.id;
      return this.model.replaceById(id, data).then(_util.jsonify).then(select).catch(function () {
    ...
    value: function patch(id, data, params) {
      ...
      }).then(_util.jsonify).then(select).catch(function () {
        return Promise.reject(new _feathersErrors2.default.NotFound('No record found for id \'' + id + '\''));
      });
    }
  }, {
