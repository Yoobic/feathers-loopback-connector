...
function shouldSetValue(value, path) {
  return (!(0, _lodash6.default)(value) || value === null || (typeof value === 'undefined' ? 'undefined' : _typeof(value)) !== 'object' || (0, _lodash4.default)(value)) && path.indexOf(null) === -1;
}
...
function coerceQueryToLoopbackFilter(query, idProp) {
  ...
  if (!(0, _lodash4.default)(query.$sort)) {
    filter.order = coerceSortToOrder(query.$sort);
  }
  if (!(0, _lodash4.default)(query.$select)) {
    filter.fields = coerceSelectToFields(query.$select, idProp);
  }
  if (!(0, _lodash4.default)(query.$include)) {
